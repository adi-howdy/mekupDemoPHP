...
if(!empty($_POST["date"])){
    ...
    //$sql_customer_count = "select shift, count(*) from customer where location = '" .  $_POST['location'] . "' and date = '" . $_POST['date'] .   "' group by shift;";

    $sql_shift = "select distinct(shift) from schedule where location = '" .  $_POST['location'] . "' and date = '" . $_POST['date'] . "';";


    echo "sql here is ..... $sql_shift";
    $result_Shift = mysqli_query(dbConnect(), $sql_shift);
    //$result_customer_count =  mysqli_query(dbConnect(), $sql_customer_count);
    $options_date = "";
    $options_shift = "";
